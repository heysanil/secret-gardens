...
describe("classifyToken", () => {
  test("classifies service tokens", () => {
    expect(classifyToken("safe_st_abc123")).toBe("service");
  });

  test("classifies user tokens", () => {
    expect(classifyToken("safe_ut_abc123")).toBe("user");
  });

  test("returns null for unknown prefixes", () => {
    expect(classifyToken("ghp_abc123")).toBe(null);
    expect(classifyToken("safe_xx_abc123")).toBe(null);
  ...
  test("is case-sensitive", () => {
    expect(classifyToken("SAFE_ST_abc123")).toBe(null);
  });

  test("returns null for an incomplete prefix", () => {
    expect(classifyToken("safe_st")).toBe(null);
    expect(classifyToken("safe_ut")).toBe(null);
  });

  test("a bare prefix still classifies by prefix", () => {
    expect(classifyToken("safe_st_")).toBe("service");
    expect(classifyToken("safe_ut_")).toBe("user");
  });
});
