...
const KEK_CHECK_CONSTANT = createHash("sha256")
  .update("secret-gardens-kek-check-v1")
  .digest();

/** Plays the role of projectId in wrapDek's AAD (`secret-gardens-dek:kek-check`). */
 ...
 * GARDENS_MASTER_KEY aborts startup with a precise error instead of runtime 500s.
 */
    ...
    if (err instanceof KekMismatchError || err instanceof DekUnwrapError) {
      throw new KekCheckError(
        "GARDENS_MASTER_KEY does not match the key this database was initialized with. " +
          ...
          "original GARDENS_MASTER_KEY (or, if you rotated the KEK, finish the rotation " +
